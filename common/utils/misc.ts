...
const safelyPrintError = (error: unknown) =>
  typeof error === "string" ? error : error.message || "An error occurred";

const sortByKey = (
  list: any[],
  keyExtractor: (props: any) => string | number
) =>
  list.sort((a, b) => {
...
const sortByMonthKeyExtractor =
  (keyExtractor: (props: any) => string) => (item: any) =>
...
const getTransactionValue =
  ({
    transaction,
    provider = TransactionProvider.XTB,
  }: GetTransactionValueType) =>

...
function GoalAchievementSection() {
  ...
  const lastNotAchievedGoalIndex = parsedGoals.findIndex(
    (goal) => !goal.isGoalAchieved
  );
  const lastAchievedGoal =
    parsedGoals[lastNotAchievedGoalIndex - 1]?.amount || 0;
  ...
  return (
    <Card gap="s16">
      <Spacer gap="s4">
        ...
        />
        <Spacer direction="top" gap="s8" size="s8">
          <Text
            variant="h5"
            letterSpacing={1}
            textTransform="uppercase"
      ...
      <Spacer gap="s16">
        <View>
          ...
          <ListItem
            leftText="Next goal"
            rightText={`${formatValue(nextGoalAmount, currency)} / month`}
          />
        </View>
        <Spacer gap="s8">

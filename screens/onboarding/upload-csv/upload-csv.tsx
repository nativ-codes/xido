    ...
    try {
      ...
      if (response.data.length) {
        if (validateColumnTitles(response.data[0])) {
          const parsedTransactionsForTotalCompanies =
            parseTransactionsForTotalCompanies(response.data);

          Store.setSymbols(parsedTransactionsForTotalCompanies);
          Store.setRawTransactions(response.data);

          setUploadedTransactions(
            `${parsedTransactionsForTotalCompanies?.length} companies found.`
          );
          setIsError(false);
        } else {
          setIsError(true);
          setUploadedTransactions(
            "Invalid column titles. Please make sure the file has the correct format."
          );
        }
      } else {
        setIsError(true);
        setUploadedTransactions(
          "Unable to parse the file. Please make sure the file has the correct format."
        );
      }
    } catch (error) {
  ...
  return (
    ...
    >
      <View style={styles.content}>
        ...
        >
          <Text>
            Go to the XTB platform, export your transaction history as a CSV
            file, and then upload the CSV file here.
          </Text>
        </MotiView>
        <MotiView
          {...bottomSlideInYLongAnimation}
          delay={MEDIUM_ANIMATION_DURATION}
        >
          <TouchableOpacity

...
function FetchCompanies() {
  ...
  const storedSymbols = shouldUseMockedData
    ? mockedSymbols
    : Store.getSymbols();
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        Analytics.sendEvent(
          Analytics.events.error_fetch,
          safelyPrintError(error)
        );
        router.navigate("/error-fetching");
      }
    })();
  ...
  const handleOnPress = () => {
    setDataByCompanyData({
      companies: companiesData as CompanyData[],
      storedSymbols,
    });
    setError("");
    router.navigate("/all-set");
  ...
  return (
    <ScreenLayout canGoBack center={<Progress previousValue={60} value={80} />}>
      {error ? (
        ...
        <View
          style={StyleSheet.compose(StyleSheet.absoluteFill, styles.loading)}
        >

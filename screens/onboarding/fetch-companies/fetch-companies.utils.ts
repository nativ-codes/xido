...
export const setDataByCompanyData = ({
  companies,
  storedSymbols,
}: SetDataByCompanyDataType) => {
  const storedRawTransactions = Store.getRawTransactions();
  const filteredRawTransactions = filterRawTransactions(
    storedRawTransactions,
    storedSymbols
  );

  const parsedTransactions = parseTransactions(filteredRawTransactions);
  const dividend = parseTransactionsForLast12MonthsDividend(
    filteredRawTransactions
  );
  // console.log("dividend", JSON.stringify(filteredRawTransactions));
  const parsedTransactionsForLast12MonthsDividend =
    getLast12MonthsDividend(dividend);
  const parsedTransactionsForCalendar = parseTransactionsForCalendar(
    filteredRawTransactions
  );
...
export const getErrorMessage = (missingSymbols: string[]) => {
  ...
  return `We couldn't find data for the following ${companyText}: ${missingSymbols.join(
    ", "
  )}.\nWould you like to proceed without ${
    missingSymbols.length === 1 ? "it" : "them"
  }?`;
};

...
const sortKeyExtractor = ([
  symbol,
  { totalDividends },
]: SortKeyExtractorPropTypes) => totalDividends;
    ...
    <TabScreenLayout
      emptyPlaceholder={
        Boolean(currency) ? (
          <EmptyPlaceholder
            shouldHideButton
            title="No dividend records found in the uploaded portfolio."
          />
        ) : undefined
      }
          ...
          <Card>
            ...
            {Boolean(currentMonth?.stats?.totalDividends) && (
              <ListItem
                ...
                rightText={formatPercentValue(
                  (currentMonth?.stats?.totalDividends /
            ...
            {Boolean(calendar?.[selectedYear]?.stats?.expectedDividends) && (
              <ListItem
                ...
                rightText={formatPercentValue(
                  ((currentMonth?.stats?.totalDividends ||
                    currentMonth?.stats?.expectedDividends ||
                    0) /
                    (calendar?.[selectedYear]?.stats?.totalDividends +
                      (calendar?.[selectedYear]?.stats?.expectedDividends ||
                        1))) *
                    100
                )}
              />
            ...
            {Boolean(currentMonth?.stats?.expectedDividends) ? (
              <ListItem
                ...
                rightText={formatValue(
                  currentMonth?.stats?.expectedDividends || 0,
                  currency
                )}
              />
            ) : (
            ...
            {Boolean(calendar?.[selectedYear]?.stats?.expectedDividends) && (
              <ListItem
                ...
                rightText={formatValue(
                  calendar?.[selectedYear]?.stats?.totalDividends +
                    (calendar?.[selectedYear]?.stats?.expectedDividends || 0),
                  currency
                )}
              />
            )}
            ...
            {sortByKey(
              Object.entries(currentMonth?.data || {}),
              sortKeyExtractor
            ).map(([symbol, { totalDividends }]: SortKeyExtractorPropTypes) => (
              <ListItem
                key={symbol}
                leftText={symbol}
                rightText={formatValue(totalDividends, currency)}
              />
            ))}
          </Card>
        </View>

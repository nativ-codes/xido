...
function Company() {
  ...
  const showModal = (section: CompanyInfoSections) => () =>
  ...
  const overallSection = useMemo(
    () => (
      <>
        <Card.Title
          title="Overall"
          onPress={showModal(CompanyInfoSections.OVERALL)}
        />
        ...
        <ListItem
          ...
          rightText={`${formatValue(
            profitOrLoss,
            currency
          )} (${formatPercentValue(profitOrLossPercentage)})`}
          variant={
            Number(profitOrLoss) > 0
              ? ListItem.variants.PROFIT
              : ListItem.variants.LOSS
          }
        />
        <Divider />
      </>
  ...
  const sharesSection = useMemo(
    () => (
      <>
        <Card.Title
          title="Shares"
          onPress={showModal(CompanyInfoSections.SHARES)}
        />
        ...
        <ListItem
          ...
          rightText={`${formatValue(
            marketVsMinShare,
            currency
          )} (${formatPercentValue(marketVsMinSharePercentage)})`}
          variant={
            Number(marketVsMinShare) > 0
              ? ListItem.variants.PROFIT
              : ListItem.variants.LOSS
          }
        />
        ...
        <ListItem
          ...
          rightText={`${formatValue(
            marketVsAvgShare,
            currency
          )} (${formatPercentValue(marketVsAvgSharePercentage)})`}
          variant={
            Number(marketVsAvgShare) > 0
              ? ListItem.variants.PROFIT
              : ListItem.variants.LOSS
          }
        />
        ...
        <ListItem
          ...
          rightText={`${formatValue(
            marketVsMaxShare,
            currency
          )} (${formatPercentValue(marketVsMaxSharePercentage)})`}
          variant={
            Number(marketVsMaxShare) > 0
              ? ListItem.variants.PROFIT
              : ListItem.variants.LOSS
          }
        />
    ...
    () =>
      Boolean(dividendsAllTime) && (
        <>
          <Card.Title
            title="Dividends"
            onPress={showModal(CompanyInfoSections.DIVIDENDS)}
          />
          <ListItem
      ...
      Boolean(dividendsAllTime) && (
        <>
          <Card.Title
            ...
            onPress={showModal(CompanyInfoSections.EXPECTED_DIVIDENDS)}
          />
          {expectedDividends.months.map((period: string) => (
            <ListItem
              ...
              leftText={period}
              rightText={formatValue(
                expectedDividends.value * shares,
                currency
              )}
            />
          ))}
    ...
    () => (
      <>
        <Card.Title
          ...
          onPress={showModal(CompanyInfoSections.LATEST_TRANSACTIONS)}
        />
        {latestTransactions.map(
          ({ date, type, amount }: TransactionsToDisplayPropTypes) => (
            <ListItem
  ...
  return (
    <ScreenLayout canGoBack>
      <Spacer direction="horizontal" size="s16">
        ...
        <Spacer direction="top" size="s32" gap="s16">
          <Card>
            {overallSection}
            {sharesSection}
            {dividendsSection}
